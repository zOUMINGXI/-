...
#     # writer.close()
# # get_course_id_index()
def get_user_id_name_courseid_index():#提取用户id，name，course_order并创建索引
    data=read_json("./scripts/entities/user.json")
    data=data[["id","name","course_order"]]
    data = data.reset_index()
    return data
# get_user_id_name_courseid_index()
def get_concept_id_name_index():#提取概念id，name，并创建索引
...
def get_course_id_name_index():#提取课程id name，field，并创建索引
    data=read_json("./scripts/entities/course.json")
