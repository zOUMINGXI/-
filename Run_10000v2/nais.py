...
class NAIS(nn.Module):
    ...
    def __init__(self, dataset):
        ...
        self.weight_size = 64
        self.algorithm = 'prod'
        self.reg_weights = [1e-7, 1e-7, 1e-5]
        self.alpha = 0
        self.beta = 0.5
        self.split_to = 0
        ...
        self.item_src_embedding = nn.Embedding(self.n_items+100000, self.embedding_size).to(self.device)
        self.item_dst_embedding = nn.Embedding(self.n_items+100000, self.embedding_size).to(self.device)
        self.bias = nn.Parameter(torch.zeros(self.n_items+100000)).to(self.device)
        if self.algorithm == 'concat':
            self.mlp_layers = MLPLayers([self.embedding_size * 2, self.weight_size]).to(self.device)
        elif self.algorithm == 'prod':
            self.mlp_layers = MLPLayers([self.embedding_size, self.weight_size]).to(self.device)
        else:
            raise ValueError("NAIS just support attention type in ['concat', 'prod'] but get {}".format(self.algorithm))
        self.weight_layer = nn.Parameter(torch.ones(self.weight_size, 1)).to(self.device)
    ...
    def attention_mlp(self, inter, target):
        ...
        if self.algorithm == 'prod':
            ...
            mlp_input = torch.cat([inter, target.unsqueeze(1).expand_as(inter)],
                                  dim=2).to(self.device)  # batch_size x max_len x embedding_size*2
